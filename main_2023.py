...
if __name__ == '__main__':
    ...
    data_root = '/workspace/data/Preprocess_HTN/datas_/'
    ALL_data = pd.read_csv('/workspace/data/Preprocess_HTN/All_data_QC.csv',low_memory=False)
    ...
    ALL_data = ALL_data[(~ALL_data['age'].isnull())] #ID NULL
    ...
    df_merge = df_merge.drop_duplicates(subset=['ID'],keep='last')#删除重复的
    ...
    # tv_df, test_df = train_test_split(ALL_data_buffer, test_size=0.2,stratify=ALL_data_buffer['label'])
    
    L2_list = [0.007,]
    BS_list = [128,128,128,128,128,128,128,128,128,128]
    random_seed_list = [3407,2022,2021,2020,2019,2018,2023,115114]
    
    test_dataset = ECGHandle.ECG_Dataset(data_root,test_df)
    for i in range(len(L2_list)):
        ...
        #         Net.MLBFNet_GUR_o(True,True,True,2,Dropout_rate=0.3),
        #        ] # type: ignore
        NET = [ res1d.resnet50(input_channels=12, inplanes=32, num_classes=2),
                res1d.resnet50(input_channels=12, inplanes=32, num_classes=2),
                res1d.resnet50(input_channels=12, inplanes=32, num_classes=2),
                res1d.resnet50(input_channels=12, inplanes=32, num_classes=2),
                res1d.resnet50(input_channels=12, inplanes=32, num_classes=2),
                res1d.resnet50(input_channels=12, inplanes=32, num_classes=2),
        ...
        for fold in range(FOLDS):
            ...
            validate_df,tarin_df = Pair_ID(tv_df_buffer,0.2,star_index=validate_start_index,Range_max=20,pair_num=1)
            validate_dataset = ECGHandle.ECG_Dataset(data_root,validate_df)
            
            train_pair_df,_ = Pair_ID(tarin_df,1,star_index=0,Range_max=20,pair_num=1,shuffle=True)
