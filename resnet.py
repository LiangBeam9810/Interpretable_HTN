...
class ResNet(nn.Module):
    def __init__(self,block,layers,num_class):
        ...
        #stem的网络层
        self.conv1=nn.Conv2d(1,self.inplane,kernel_size=7,stride=(1,2),padding=3,bias=False)
        ...
        self.stage2=self.make_layer(self.block,128,layers[1],stride=4)
        self.stage3=self.make_layer(self.block,256,layers[2],stride=4)
        self.stage4=self.make_layer(self.block,512,layers[3],stride=4)

        #后续的网络
        self.avgpool=nn.AvgPool2d((12, 20))
    ...
    def make_layer(self,block,plane,block_num,stride=1):
        ...
        if(stride!=1 or self.inplane!=plane*block.extention):
            downsample=nn.Sequential(
                nn.Conv2d(self.inplane,plane*block.extention,stride=(1,stride),kernel_size=1,bias=False),
        ...
        #Conv_block
        conv_block=block(self.inplane,plane,stride=(1,stride),downsample=downsample)
